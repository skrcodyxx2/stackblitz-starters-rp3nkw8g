/*
  ...
  1. Insert default company settings
  2. Insert menu categories with data
  3. Insert menu items with data
  4. Insert gallery albums and images
  5. Insert special menus
  6. Handle all missing data scenarios
*/
...
((SELECT id FROM menu_categories WHERE name = 'Accompagnements' LIMIT 1), 'Bananes Pesées', 'Bananes plantains frites, accompagnement traditionnel', 6.99, 'https://images.pexels.com/photos/1729797/pexels-photo-1729797.jpeg?auto=compress&cs=tinysrgb&w=400&h=300&fit=crop', ARRAY['Bananes plantains', 'Huile'], ARRAY[], 15, 220, true, false, 2),
((SELECT id FROM menu_categories WHERE name = 'Accompagnements' LIMIT 1), 'Légumes Créoles', 'Mélange de légumes sautés aux épices', 9.99, 'https://images.pexels.com/photos/1640772/pexels-photo-1640772.jpeg?auto=compress&cs=tinysrgb&w=400&h=300&fit=crop', ARRAY['Légumes variés', 'Épices créoles'], ARRAY[], 20, 150, true, false, 3),

-- Desserts
((SELECT id FROM menu_categories WHERE name = 'Desserts' LIMIT 1), 'Doukounou', 'Pudding de maïs traditionnel haïtien', 7.99, 'https://images.pexels.com/photos/1181396/pexels-photo-1181396.jpeg?auto=compress&cs=tinysrgb&w=400&h=300&fit=crop', ARRAY['Maïs', 'Lait de coco', 'Épices'], ARRAY[], 45, 320, true, false, 1),
...
-- Boissons
((SELECT id FROM menu_categories WHERE name = 'Boissons' LIMIT 1), 'Jus de Canne', 'Jus de canne à sucre frais', 4.99, 'https://images.pexels.com/photos/3184183/pexels-photo-3184183.jpeg?auto=compress&cs=tinysrgb&w=400&h=300&fit=crop', ARRAY['Canne à sucre'], ARRAY[], 5, 120, true, false, 1),
...
((SELECT id FROM menu_categories WHERE name = 'Boissons' LIMIT 1), 'Thé Glacé Tropical', 'Thé glacé aux fruits tropicaux', 3.99, 'https://images.pexels.com/photos/1640772/pexels-photo-1640772.jpeg?auto=compress&cs=tinysrgb&w=400&h=300&fit=crop', ARRAY['Thé', 'Fruits tropicaux'], ARRAY[], 10, 80, true, false, 3)
ON CONFLICT DO NOTHING;
